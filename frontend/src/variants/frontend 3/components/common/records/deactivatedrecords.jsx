...
const DeactivatedRecords = () => {
  ...
  const handleSelectAll = (e) => {
    setSelectedRecords(e.target.checked ? deactivatedStudents.map((s) => s.id) : []);
  };
            ...
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-6 py-4 text-left w-12">
                  <input
                    type="checkbox"
                    checked={selectedRecords.length === deactivatedStudents.length}
                    onChange={handleSelectAll}
                    className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 w-4 h-4 cursor-pointer"
                  />
                </th>
                {['Student Name', 'Student ID', 'Last Grade Level', 'Deactivation Reason', 'Deactivation Date', 'QR Status', 'Actions'].map((heading, i) => (
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {deactivatedStudents.map((student) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-gray-700 font-medium">{student.studentId}</td>
