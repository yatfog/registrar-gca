...
const ProcessExitModal = ({ isOpen, onClose, student }) => {
  ...
  const handleArchiveRecord = () => {
    const isCleared =
      exitData.libraryCleared &&
      exitData.financeCleared &&
      exitData.propertyCleared &&
      exitData.advisoryCleared;
    ...
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-start justify-center p-4 md:p-10 z-50 overflow-y-auto">
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl mt-10 p-6 md:p-8 transform transition-transform duration-300">
        ...
        <div className="flex justify-between items-center bg-yellow-400 text-gray-900 font-extrabold text-lg md:text-xl p-4 -mx-6 -mt-6 md:-mx-8 md:-mt-8 rounded-t-2xl shadow-md">
          ...
          <button
            onClick={onClose}
            className="text-gray-900 hover:text-black/80 text-3xl leading-none"
          >
            &times;
          </button>
        </div>
        ...
        <div className="p-4 pt-6 text-gray-800">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-2 mb-6 text-sm">
            <h4 className="col-span-1 lg:col-span-3 text-md font-extrabold text-blue-600 mb-2 border-b border-blue-100 pb-1">
              Student and Exit Details
            </h4>

            <p>
              <span className="font-bold">Student Name:</span>{" "}
              {exitData.studentName}
            </p>
            <p>
              <span className="font-bold">Parent / Guardian:</span>{" "}
              {exitData.parentGuardian}
            </p>
            <p>
              <span className="font-bold">Exit Type:</span> {exitData.exitType}
            </p>
            ...
            <p>
              <span className="font-bold">Contact:</span> {exitData.contact}
            </p>
            <p>
              <span className="font-bold">Request Date:</span>{" "}
              {exitData.requestDate}
            </p>

            <p>
              <span className="font-bold">Grade & Section:</span>{" "}
              {exitData.lastGradeCompleted} - Morning
            </p>
            <p>
              <span className="font-bold">Last Attendance:</span>{" "}
              {exitData.lastAttendance}
            </p>
            <p>
              <span className="font-bold">New School:</span>{" "}
              {exitData.newSchool}
            </p>

            <p className="lg:col-span-3 mt-2">
              <span className="font-bold">Exit Reason:</span>{" "}
              {exitData.exitReason}
            </p>
          </div>
          ...
          <div className="mb-6 bg-yellow-50 p-6 rounded-xl border border-yellow-200 shadow-inner">
            ...
            <div className="space-y-3">
              {[
                {
                  label: "Library - No outstanding books",
                  name: "libraryCleared",
                  checked: exitData.libraryCleared,
                },
                {
                  label: "Finance - No outstanding balances",
                  name: "financeCleared",
                  checked: exitData.financeCleared,
                },
                {
                  label: "Property - All school materials returned",
                  name: "propertyCleared",
                  checked: exitData.propertyCleared,
                },
                {
                  label: "Advisory Teacher - Clearance Approved",
                  name: "advisoryCleared",
                  checked: exitData.advisoryCleared,
                },
              ].map((item) => (
                <label
                  key={item.name}
        ...
        {students.length === 0 && (
          <div className="text-center py-12 bg-white rounded-xl shadow-md mt-6">
            ...
            <p className="text-gray-600">
              No pending exit processing requests at the moment.
            </p>
          </div>
        )}
      </div>
